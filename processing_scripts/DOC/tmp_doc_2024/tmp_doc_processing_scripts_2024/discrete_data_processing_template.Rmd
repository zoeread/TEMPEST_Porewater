...
#input AMPs function for reading in the data 
...
#Rename columns
colnames(dat) <- c( "Sample.Name", "Inj_No","Analysis","Area", "Mean Area", "Conc", "Inst_Results")
head(dat)

#Pull out computer calculated concentrations
colmn <- paste("col", 1:2)
dat <- dat %>% separate(Inst_Results, sep = " ", into = colmn, remove = FALSE)
dat<- dat %>%
  rename("NPOC_CONC" = "col 1",
          "TN_CONC" = "col 2")
dat$NPOC_CONC <- gsub('[NPOC:]','',dat$NPOC_CONC)
dat$NPOC_CONC <- gsub('[mg/L]','',dat$NPOC_CONC)
dat$NPOC_CONC <- as.numeric(dat$NPOC_CONC)
dat$TN_CONC <- gsub('[TN:]','',dat$TN_CONC)
dat$TN_CONC <- gsub('[mg/L]','',dat$TN_CONC)
dat$TN_CONC <- as.numeric(dat$TN_CONC)


#Separate TOC and TN files
TOC_dat <- subset(dat, Analysis == "NPOC")
TN_dat <- subset(dat, Analysis == "TN")
