...
stds <- read_plate(
  file = "TEMPEST_FE_20230530_Stds1.csv",             # full path to the .csv file
...
dat <- read_plate(
  file = "TEMPEST_FE_20230530_Samples1.csv",             # full path to the .csv file
  well_ids_column = "Wells",    # name to give column of well IDs (optional)
  sep = ","                     # separator used in the csv file (optional)
)
str(dat)
...
## Check the dups for QAQC - Should be below 10%  
```{r}
...
## Check the spks for QAQC (should be between 100-120%) - PLZ DISREGARD SPKS - WORKING THIS OUT
```{r}
...
head(dat_IDs)
write.csv(dat_IDs, file="TEMPEST_FE_20230530_Summary_Data.csv")

#Read out all the data in dat 
head(dat)
write.csv(dat, file="TEMPEST_FE_20230110_Data.csv")
...
dat2 <- dat2[ ,-(2:9)]
head(dat2)
write.csv(dat2, file="TEMPEST_FE_20230110_short_Data.csv")
